...
export type DioramaCanvasHandle = { zoom: (factor: number) => void; reset: () => void; focusCoordinate: (coordinates: readonly [number, number]) => void };
...
const DioramaCanvas = forwardRef<DioramaCanvasHandle, Props>(function DioramaCanvas(props, ref) {
  ...
  useImperativeHandle(ref, () => ({ zoom: factor => renderer.current?.zoom(factor), reset: () => renderer.current?.reset(), focusCoordinate: coordinates => renderer.current?.focusCoordinate(coordinates) }), []);
  ...
  return <div className="diorama-canvas" data-testid="diorama-stage" data-status={status}>
    <div ref={host} className="diorama-webgl" />
    {status === 'loading' && <div className="diorama-loading" role="status"><span className="diorama-loading-cube" aria-hidden="true" /><strong>성남의 작은 풍경을 준비해요</strong><span>잠시 후 직접 돌려볼 수 있어요.</span></div>}
