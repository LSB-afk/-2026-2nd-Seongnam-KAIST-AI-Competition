...
// The default audience is youth, so bars and pubs stay out of both food lists.
const FOOD_CATEGORIES = ['음식점','카페','간편 음식','간편음식','베이커리','아이스크림'];
export function CityFoodPanel({place,selectedId,onSelect,onSelectUrban,entries}:{place:Place|null;selectedId:string|null;onSelect:(food:CityFoodPlace)=>void;onSelectUrban:(entry:UrbanDirectoryEntry)=>void;entries:readonly UrbanDirectoryEntry[]}) {
  ...
  const available=(scope==='all'||!place?CITY_FOOD_PLACES:nearby.map(item=>item.food)).filter(food=>food.category!=='bar');
  const shown=available.filter(food=>category==='all'||food.category===category);
  const selected=CITY_FOOD_PLACES.find(food=>food.id===selectedId&&food.category!=='bar');
  const local=useMemo(()=>searchUrbanDirectory(entries.filter(entry=>FOOD_CATEGORIES.includes(entry.category)&& (category==='all'||category==='cafe'&&['카페','베이커리'].includes(entry.category)||category==='restaurant'&&!['카페','베이커리'].includes(entry.category))), '', place?projectCityCoordinate(place.lng,place.lat):[0,0,0]),[entries,place,category]);
  return <section className="city-food-panel" aria-label="주변 먹거리">
    <span className="diorama-detail-kicker">먹거리도 이야기의 일부</span><h2>{scope==='nearby'&&place?`${place.name} 주변 먹거리`:'성남의 먹거리'}</h2>
    ...
    <div className="city-food-list">{shown.map(food=>{
      ...
      return <article key={food.id} data-selected={selectedId===food.id}>
        <span className={`city-food-kind ${food.category}`}>{food.category==='cafe'?'☕ 카페':'♨ 식사'}</span>
