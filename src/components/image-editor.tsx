...
import { editBlockReason } from "@/lib/budget";
...
export function CardPhoto({ card, retryable = false }: { card: Card; retryable?: boolean }) {
  ...
  return <div className="card-photo-frame"><Image key={attempt} src={image.src} alt={`${image.kind === "ai" ? "AI 생성 이미지" : image.kind === "upload" ? "사용자 업로드 이미지" : "실제 사진"}: ${card.title}`} fill unoptimized sizes="500px" onError={() => setFailed(true)} style={{ objectFit: "cover", objectPosition: `${image.crop.x * 100}% ${image.crop.y * 100}%`, transform: `scale(${image.crop.zoom})`, transformOrigin: `${image.crop.x * 100}% ${image.crop.y * 100}%` }} /><span className="image-kind">{image.kind === "ai" ? (card.imagination || image.prompt?.imagination) ? "AI 생성 이미지 · 상상 이미지" : "AI 생성 이미지 · 실제 사진 아님" : image.kind === "upload" ? card.imagination ? "상상 장면 · 사용자 업로드 이미지" : "사용자 업로드 이미지" : card.imagination ? "상상 장면 · 사진은 실제 모습" : "실제 장소 사진"}</span></div>;
}
...
export default function ImageEditor({ run, card, config, locked, onRun, onBusy }: { run: Run; card: Card; config?: ImageConfig; locked: boolean; onRun: (run: Run) => void; onBusy: (value: boolean) => void }) {
  ...
  const cardPlace = getPlace(card.placeId ?? run.brief.placeId ?? run.brief.place);
  const hasPlacePhoto = !!cardPlace?.photo;
  const blockReason = editBlockReason(run);
  ...
  async function upload(selected?: File) {
    ...
    try {
      ...
      const asset = await request<ImageAsset>("/api/images", { data, mime: selected.type, placeId: cardPlace?.id || card.image?.placeId, author: author.trim() || "사용자 제공", license: "사용자가 이용 권한을 확인한 사진" });
  ...
  return <section className="image-editor" aria-label="카드 사진 편집">
    ...
    {blockReason && <p className="stop-reason" id="image-edit-block-reason">{blockReason}</p>}
    <fieldset disabled={locked || pending || imageRunning || !!blockReason} aria-describedby={blockReason ? "image-edit-block-reason" : undefined}>
      <h3>사진 구도 조정</h3>
