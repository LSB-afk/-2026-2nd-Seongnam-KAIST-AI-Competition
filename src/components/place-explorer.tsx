  ...
  return <section className="explorer-page" hidden={!visible} aria-label={savedOnly?'저장한 장소':'관광지 탐색'}>
    ...
    <div className={`explorer-layout ${selected?'has-detail':''} mobile-${state.mobileView}`}>
      <div ref={results} className="place-results" aria-label="관광지 검색 결과" data-tour="place-results" tabIndex={-1}>
        {filtered.length?filtered.map(place=><article key={place.id} className="place-result-wrap"><button type="button" className={`place-result ${selected?.id===place.id?'selected':''}`} aria-pressed={selected?.id===place.id} onClick={()=>choose(place)}><PlacePhoto place={place}/><div><span className="place-category">{place.district} · {place.type}</span><h2>{place.name}</h2><p>{place.description}</p><span className="place-address">{place.address}</span></div></button><button className="save-place-button" type="button" aria-label={`${place.name} ${savedIds.includes(place.id)?'저장 해제':'저장'}`} aria-pressed={savedIds.includes(place.id)} onClick={()=>toggle(place.id)}>{savedIds.includes(place.id)?'♥ 저장됨':'♡ 저장'}</button></article>):<div className="explorer-empty"><h2>{savedOnly&&!savedIds.length?'저장한 장소가 아직 없어요.':'조건에 맞는 장소가 없어요.'}</h2><p>{savedOnly&&!savedIds.length?'성남 둘러보기에서 마음에 드는 장소의 저장 버튼을 눌러 주세요.':'다른 검색어나 지역으로 찾아보세요.'}</p><button className="secondary-button" type="button" onClick={()=>onStateChange(defaultExplore())}>검색 조건 초기화</button></div>}
      </div>
      ...
      {selected&&<>
        ...
        <aside ref={panel} className="place-detail" data-tour="place-detail" role={compact?'dialog':'region'} aria-modal={compact||undefined} aria-label={`${selected.name} 상세 정보`} onKeyDown={event=>{
          ...
          if(compact&&event.key==='Tab'){
            const nodes=[...(panel.current?.querySelectorAll<HTMLElement>('button:not([disabled]),a[href],input,select,[tabindex="0"]')||[])].filter(el=>el.getClientRects().length>0);
