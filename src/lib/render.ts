...
import { defaultCardImage, imageDataUri } from "./images";
...
export function cardHtml(
  ...
  const placeName = run.brief.story ? cardPlace(run, card.id).name : run.brief.place;
  const chapter = ["발견", "기록", "연결", "상상"][index] ?? "문화";
  const color = card.imagination ? "#7854AF" : "#244CC5";
  const sourceNames = [...new Set(run.sources.filter(source => source.status === "ok" && (!run.brief.story || source.placeId === cardPlace(run, card.id).id)).map(source => source.publisher))].join(" · ");
...
export async function renderCards(
  ...
  try {
    ...
    for (const card of run.cards) {
      signal.throwIfAborted();
      const place = cardPlace(run, card.id);
      const stop = storyStopForCard(run, card.id);
      if (run.brief.story && (card.placeId !== place.id || card.stopId !== stop?.id)) throw new Error("카드의 장소 배정이 일치하지 않습니다.");
      card.image ??= await defaultCardImage(run, card.id);
      const textOnlyAllowed = (place.photo === null && (run.brief.story || getPlace(run.brief.place)?.id === place.id)) || stop?.photoChoice === "none";
      const expectedPlaceId = place.id;
      if (!card.image) {
