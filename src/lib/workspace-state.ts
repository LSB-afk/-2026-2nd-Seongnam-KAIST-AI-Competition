...
export type WorkspaceView = 'dashboard' | 'explore' | 'saved' | 'studio' | 'history';
...
export type WorkspaceState = { version: 1; view: WorkspaceView; explore: ExploreState; savedIds: string[]; draft: DraftState; runId: string | null; selectedCardId: string; tab: ReviewTab; editDrafts: EditDraft[]; historyQuery: string; historyStatus: string };
...
const VIEWS: WorkspaceView[] = ['dashboard','explore','saved','studio','history'];
...
export function createWorkspace(draft: DraftState): WorkspaceState { return {version:1,view:'dashboard',explore:defaultExplore(),savedIds:[],draft:{...draft,brief:{...draft.brief}},runId:null,selectedCardId:'',tab:'evidence',editDrafts:[],historyQuery:'',historyStatus:'all'}; }
...
export function decodeWorkspace(raw:string|null,draft:DraftState):WorkspaceState {
  ...
  try {
    ...
    const restoredBrief:Brief={place:place.name,placeId:place.id,audience:string(brief.audience,draft.brief.audience,40),goal:string(brief.goal,draft.brief.goal,1000),cardCount:4,includeFuture:true,...(brief.purpose?{purpose}:draft.brief.purpose?{purpose:draft.brief.purpose}:{})};
    ...
    return {...base,view:oneOf(value.view,VIEWS,'dashboard'),explore:normaliseExplore(value.explore),savedIds:[...new Set(Array.isArray(value.savedIds)?value.savedIds.filter((id):id is string=>typeof id==='string'&&!!getPlace(id)):[])],draft:{brief:restoredBrief,mode:oneOf(savedDraft.mode,['fixture','live'],'fixture'),strategy:oneOf(savedDraft.strategy,['agent','baseline'],'agent'),scenario:oneOf(savedDraft.scenario,['normal','causal','future','mismatch','unavailable','persistent'],'normal'),imageChoice:oneOf(savedDraft.imageChoice,['photo','ai'],'photo')},runId:UUID.test(string(value.runId))?String(value.runId):null,selectedCardId:/^card-[1-4]$/.test(string(value.selectedCardId))?String(value.selectedCardId):'',tab:oneOf(value.tab,TABS,'evidence'),editDrafts,historyQuery:string(value.historyQuery,'',100),historyStatus:oneOf(value.historyStatus,STATUSES,'all')};
  } catch { return base; }
}
const ROUTE_KEYS=['view','place','district','category','theme','q','run','card','tab','lat','lng','zoom','display'];
