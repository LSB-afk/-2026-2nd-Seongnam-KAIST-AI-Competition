...
## 2026-09-15 AI 에이전트·쉬운 설명·문장 근거

성남 장소 탐색 → 공식 자료 → 카드뉴스 네 장 → 검수 → 담당자 승인 → 다운로드 흐름을 유지했다. AI 에이전트 화면은 실제 실행에 기록된 자료·이벤트·모델 호출 시도·비용·현재 검수와 담당자 승인 상태를 보여준다. 여섯 기능의 설명은 기존 도구 계약에 연결되며, 화면 안내를 맡는 타미와 콘텐츠 제작 에이전트를 구분한다. 새 npm 의존성은 없다.

쉬운 설명은 새 제작 입력에 저장되고, 기존 결과에는 사용자가 명시적으로 요청한 버전에서만 적용된다. 담당자 보호 카드·사진을 유지하며 새 버전의 검수와 승인을 다시 받는다. 단위 검증은 날짜·수치·단위·한정 표현 보존, 오래된 버전, 중복 요청, 취소 후 늦은 응답, 예산 제한과 보호 카드 동작을 포함한다. fixture는 준비한 어휘·표현 변환이며 자유 입력의 의미 이해를 입증하지 않는다. 실제 모델 응답을 흉내 낸 ‘일부 → 전부’ 손실 사례에서는 기존 카드를 보존하고 검토 필요로 끝나는 것을 확인했다. 이 검사는 모든 의미 변화를 탐지한다는 보장이 아니다.

문장 근거는 현재 버전·필드 내용과 일치하는 검수만 표시하고, 원문 인용·수집일·발행/수정일·담당자 다음 행동을 연결한다. 독립 코드 검토에서 발견한 두 항목을 보완했다. 오래된 검수를 ‘근거 확인’으로 표시하던 부분은 ‘현재 버전 재검수 필요’로 바꾸고, 편집 화면이 없는 대본에는 작동하지 않는 이동 버튼을 표시하지 않는다. 두 회귀를 테스트에 포함했다.

| 검사 | 최종 결과 |
| --- | --- |
| `npm test` | 25개 파일, 332개 통과 |
| `npm run test:e2e` | production 환경 32개 통과, 54.5초 |
| `TIMESTORY_DIST_DIR=.next-e2e npm run typecheck` | 통과 |
| `npm run lint` | 통과 |
| `TIMESTORY_DIST_DIR=.next-e2e npm run build` | 통과, 기존 개발 서버와 빌드 경로 분리 |
| `npm run eval` | fixture 12회: 각 정책 4개 승인 대기·2개 검토 필요, 실제 모델 호출 0 |
| `npm run eval:claims -- --validate` | 36개 입력·18개 holdout 데이터 검사 통과, 초안 정답·API 호출 0·평가 지표 null |

새 브라우저 검증은 AI 메뉴·뒤로 가기·새로고침·초안 유지, 실제 이벤트 목록, 쉬운 설명 변환과 사진 보존, v1 → v2 재검수, 문장에서 본문 편집으로 포커스 이동, 담당자 승인과 실제 다운로드를 수행한다. 기존 지도·타미 드래그·튜토리얼·편집 보호 흐름도 통과했다. 처음 전체 실행에서 새 메뉴와 겹친 타미 설정 선택자와 본문 포커스 선택자를 수정했으며, 최종 32개 결과에는 이 수정이 포함된다.

1440/1024/390px의 최종 에이전트 화면에 가로 넘침과 페이지 예외가 없었다. 모바일에서도 역할 설명을 읽을 수 있다. 내려받은 ZIP에서 1080×1080 PNG 네 장을 추출해 한글 제목/본문, 실제 장소 사진, 출처, 마지막 카드의 상상 표시를 직접 확인했다. fixture 실행의 API 호출은 0이다.

별도 실제 지도 확인에서는 OpenStreetMap 타일 응답 15개가 HTTP 200이었다. 이 확인은 공용 서비스의 해당 로컬 실행에 한정된다. 자동 E2E 지도는 모의 응답을 사용하며, NAVER 인증 및 실제 텍스트·이미지 API는 설정이 없어 검증하지 않았다. 쉬운 설명의 시민 이해 효과·실물 모바일·스크린리더 음성·현장 업무 절감도 측정하지 않았다.

증거 파일은 다음과 같다.

- `outputs/focused-agent/verification.json`: 실제 fixture 작업의 콘텐츠/검수 버전 2, API 호출 0, 페이지 예외 없음, PNG 4장.
- `outputs/focused-agent/final-agent-{1440,1024,390}.png`, `final-review-390.png`: 최신 반응형 화면.
- `outputs/focused-agent/easy-timestory.zip`, `card-{1,2,3,4}.png`: 승인한 실제 출력물.
- `outputs/focused-agent/map-check.json`, `actual-map-1440.png`: 실제 OSM 표시와 설정 상태.

주요 구현: [에이전트 화면](../src/components/agent-center.tsx), [쉬운 설명 변환](../src/lib/reading-style.ts), [버전별 실행 서비스](../src/lib/service.ts), [문장별 근거](../src/components/review-trace.tsx), [브라우저 회귀](../tests/agent-workspace.e2e.spec.ts).
